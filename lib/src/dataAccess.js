module.exports = ({ persistence, validator, downloader, config, logger, utils }) => {
  ...
  async function initData(force = false) {
    ...
    if (!init || force) {
      await persistence.initData(schemas, indexes)
        .then((result) => {
          schemaCache = JSON.parse(JSON.stringify(result.schemas));
          indexCache = JSON.parse(JSON.stringify(result.indexes));
          indexTypes = Object.keys(indexCache);
          logger.log(`Init schema complete`, { schemaCache, indexCache });
          result.data.forEach((dataResult) => {
            dataCache[dataResult.modelClass] = JSON.parse(JSON.stringify(dataResult.data));
  ...
  function cacheRecord(modelClass, record) {
    ...
    writeData(modelClass, writableData);
  }
  ...
  function filterData(modelClass, filters) {
    ...
    const filteredData = filterFromIndexes(modelClass, modelData, filterIndexes, filterSchemas, filters);
    return filteredData.reduce((arr, [id, record]) => {
      arr.push(record);
      return arr;
    }, []);
  }
  ...
  function filterValueMatch(filterSchemas, field, indexedValue, filterValue) {
    ...
    const multiMatch = Array.isArray(indexedValue) ?
      Array.isArray(filterValue) && filterValue.some(fv => indexedValue.includes(fv)) :
      Array.isArray(filterValue) && filterValue.some(fv => fv === indexedValue);
    ...
    ) && rangeMatch(filterValue.startTime, filterValue.endTime, indexedValue);

    return partialMatch || multiMatch || exactMatch || dateRangeMatch || timeRangeMatch;
  }

  function rangeMatch(rangeStart, rangeEnd, compareValue) {
    return (utils.isEmpty(rangeStart) || new Date(compareValue) >= new Date(rangeStart)) &&
      (utils.isEmpty(rangeEnd) || new Date(compareValue) <= new Date(rangeEnd));
  }

  function filterFromIndexes(modelClass, modelData, indexes, filterSchemas, filters) {
    return Object.entries(modelData).filter(([id, record]) => {
      return Object.entries(filters).every(([field, filterValue]) => {
        const recordValue = record[field];

        return filterFromIndex(indexes, field, filterValue, id) ||
          filterFromData(filterSchemas, field, filterValue, recordValue);
      });
    });
  }

  function filterFromIndex(indexes, field, filterValue, id) {
    return utils.notEmpty(indexes[field]) &&
      utils.notEmpty(indexes[field][filterValue]) &&
      indexes[field][filterValue].includes(id);
  }

  function filterFromData(filterSchemas, field, filterValue, recordValue) {
    return utils.notEmpty(recordValue) && filterValueMatch(filterSchemas, field, recordValue, filterValue);
  }
  ...
  function removeUniqueIndexes(modelClass, record) {
    const uniqueIndexes = indexCache.unique;
    if (utils.isEmpty(uniqueIndexes[modelClass])) {
      return;
    }
