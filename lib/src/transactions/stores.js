...
module.exports = ({ dataAccess, utils, logger }) => {
  ...
  function create(params) {
    if (utils.notEmpty(params.currencyId) && utils.isEmpty(params.homeCurrencyAmount)) {
  ...
  function reverseTransaction(id) {
    ...
    if (utils.notEmpty(record.associatedTransactionId)) {
      return {
        success: false,
        errors: {
          id: ['alreadyReversed'],
    ...
    if (success) {
      const originalTransaction = results[0].result.record;
      const reversalTransaction = results[1].result.record;
      const includes = {
