...
module.exports = ({ dataAccess, logger, utils }) => {
  ...
  function prefillReceipt({ invoice, pastReceipts, receiptConfig, receiptNumberSequence, billingContact, currency }) {
    ...
    if (pastReceipts.length > 0) {
      paidAmount = pastReceipts.reduce((sum, pastReceipt) => {
        sum = sum + pastReceipt.paymentAmount;
        return sum;
      }, 0);
    }
  ...
  function viewInvoiceWithIncludes(invoiceId) {
    ...
    const pastReceipts = dataAccess.list(
      'income_receipts',
      {
        filters: { invoiceId },
  ...
  function viewReceiptWithIncludes(receiptId) {
    const receipt = dataAccess.view(
      'income_receipts',
      receiptId,
      { include: ['incomeTransactionId'] }
    ).record;
  ...
  function createWithTransaction(params) {
    try {
      ...
      const receiptConfigId = params.incomeReceiptConfigId;
      ...
      const paidAmount = parseFloat(pastReceipts.reduce((sum, pastReceipt) => {
        return sum + pastReceipt.paymentAmount;
      }, 0).toFixed(2));
      const billableAmount = invoice.totalAmount;
      const payableAmount = parseFloat((billableAmount - paidAmount).toFixed(2));

      if (receipt.paymentAmount > payableAmount) {
        return {
          success: false,
          errors: {
      ...
      receipt.remainingAmount = parseFloat((payableAmount - receipt.paymentAmount).toFixed(2));
        ...
        [
          ...
          },
          {
            ...
            params: {
              ...
              description: `Income from Receipt ${lastUsedNumber}`,
          ...
          {
            id: 'createReceipt',
            params: Object.assign({}, receipt, { incomeReceiptConfigId: receiptConfigId }),
            invoke: (params, pastResults) => {
              ...
              return dataAccess.create(
                ...
                Object.assign(
                  ...
                  params,
                  {
                    receiptNumber: lastUsedNumber.toString(),
                    incomeTransactionId: createdTransaction.id,
                  }
                )
              );
            },
      ...
      if (success) {
        const createdReceipt = results[0].result.record;
        const transaction = results[1].result.record;
        return {
          success: true,
